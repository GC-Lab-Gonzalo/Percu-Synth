...
| Archivo                          | Qué es                                                               |
|----------------------------------|----------------------------------------------------------------------|
| `oscilador_escalas_esp32s3.bin`  | Imagen **merged** (bootloader + particiones + boot_app0 + app), ~550 KB |
...
- **Desde el navegador (PC, Chrome/Edge):** copia `oscilador_escalas_esp32s3.bin` a
...
- **Desde la terminal:** `esptool --chip esp32s3 write-flash 0x0 oscilador_escalas_esp32s3.bin`
