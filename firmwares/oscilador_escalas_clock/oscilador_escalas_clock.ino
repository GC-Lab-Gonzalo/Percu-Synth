// ==============================================================================================================================================
// PERCU-SYNTH — OSCILADOR DE ESCALAS + RELOJ MIDI (dron de 4 osciladores, IMU → filtro, MIDI Clock por el DIN-5) — GC Lab Chile
// ==============================================================================================================================================
...
// - LED RGB direccionable del módulo ESP32-S3 (DevKitC-1) |DATA -> 48| (color de la nota que suena)
...
//
// El MIDI sale por Serial1 enrutado al GPIO 43, NO por Serial0: Serial0 comparte ese TX con
// el DIN-5 y su RX es el 44 (BTN1). Este firmware no imprime nada por ningún puerto.
// ==============================================================================================================================================
...
//
// Esta variante ("clock") le suma tres cosas al oscilador_escalas base:
//
//   · RELOJ MIDI por el DIN-5: MIDI Clock a 24 PPQN al tempo del tap (BTN4), y el
//     transporte sigue a la INTERMITENCIA (BTN3): al encenderla sale Start, al apagarla
//     Stop. Cada tap mientras corre reengancha el "1" del sinte externo con Stop+Start
//     (si no, su secuencia queda corrida contra el corte para siempre). El reloj se
//     CUENTA con el contador de muestras del audio (no puede irse de fase con el corte)
//     pero se MANDA desde la tarea de control a 1 kHz (mandarlo desde el render mete
//     ±2,9 ms de fluctuación por pulso). Los ticks salen siempre, aun parado, para que
//     el sinte ya tenga el tempo cuando llegue el Start.
//   · Los 6 LEDs de la placa PRENDEN AL RITMO de la intermitencia: siguen la misma
//     envolvente que corta el audio (se ven exactamente como suenan).
//   · El LED RGB del módulo muestra el COLOR DE LA NOTA: recorre los osciladores que
//     están sonando, uno por pulso del tempo (el mismo color que el LED 0..3 de ese
//     oscilador), y con la intermitencia activa late con ella.
//
...
//          ON = MIDI Start por el DIN-5 · OFF = MIDI Stop
// - BTN4 → TAP TEMPO de la intermitencia y del MIDI Clock (marca 2 o más golpes; también
//          reengancha la fase, así que el corte cae donde tú marcas y el sinte externo
//          recibe Stop+Start para caer en el mismo "1"). Sin tocarlo: 120 BPM
...
//                   Con la intermitencia activa, los 6 prenden y se apagan con el corte.
// LED del módulo:   color de la nota de uno de los osciladores que suenan; cambia de
//                   oscilador en cada pulso del tempo. Apagado si no suena nada.
// MIDI DIN-5:       Clock 24 PPQN siempre · Start al activar la intermitencia · Stop al
//                   apagarla · Stop+Start en cada tap mientras corre.
// ==============================================================================================================================================
...
//                     midiTicks  midiStartTick  reqMidiStart
...
// Reloj MIDI: el audio CUENTA los ticks (24 por pulso, con el mismo contador que el corte)
// y la tarea de control los MANDA. Al reenganchar el "1" con la intermitencia activa, el
// audio marca en qué tick cae el pulso y pide Stop+Start: el control descarta los ticks
// anteriores al pulso y manda el Start justo antes del tick del "1".
...
uint32_t tickAcc = 0;                  // acumulador (tarea de audio): 24 por muestra vs gatePeriod
...
void tapTempo() {
  ...
  // como si fuera un pulso y arrastraba el tempo durante varios taps — y al sinte
  // externo le llegaba un clock a otro tempo).
...
// Con la intermitencia activa los 6 siguen la envolvente del corte: prenden al ritmo.
// LED del módulo (GPIO 48): color de la nota de un oscilador que suena; en cada pulso
// del tempo pasa al siguiente oscilador activo. Mismo color que su LED 0..3.
...
void renderLEDs() {
  ...
  static uint32_t rotIdx = 0;                      // qué oscilador muestra el LED del módulo
  if (t - lastFrame < LED_REFRESH_MS) return;
  lastFrame = t;

  uint32_t bc = beatCount;
  if (bc != lastBeat) { lastBeat = bc; beatPulse = 1.0f; rotIdx++; }
  bool  gOn  = gateOn;
  float gEnv = gateEnvOut;
  ...
  if (beatPulse > 0.02f) {
    uint8_t p = (uint8_t)(beatPulse * (gOn ? 220.0f : 90.0f));
    leds[5] += CRGB(p, p, p);
  }
  beatPulse *= 0.55f;

  // ── LED del módulo: color de la nota (recorre los osciladores activos, uno por pulso) ──
  int activos[NUM_SLOTS]; int nAct = 0;
  for (int s = 0; s < NUM_SLOTS; s++)
    if (slots[s].note >= 0 && slots[s].amp > 0.02f) activos[nAct++] = s;
  if (nAct > 0) {
    int s = activos[rotIdx % (uint32_t)nAct];
    onboard[0] = CHSV(noteHue(slots[s].note), 255, 255);
  } else {
    onboard[0] = CRGB(0, 0, 0);                    // no suena nada → apagado
  }

  // ── Intermitencia: la tira y el LED del módulo prenden AL RITMO del corte ──
  if (gOn) {
    uint8_t k = (uint8_t)(gEnv * 255.0f);
    for (int i = 0; i < NUM_LEDS; i++) leds[i].nscale8(k);
    onboard[0].nscale8(k);
  }
...
void pasoControl() {
  ...
  if (b3 == LOW && b3Level == HIGH && (tms - b3Time) > DEBOUNCE_MS) {
    b3Time = tms;
    gateOn = !gateOn;
    if (gateOn) reqResync = true;              // arranca abierto, en el pulso → Start (lo pide el audio)
    else if (ENVIAR_MIDI_CLOCK) { MIDIOUT.write(MIDI_STOP); midiRunning = false; }
    flashLevel = 0.5f;
  }
  // BTN4 → tap tempo
...
void enviarMidi() {
  ...
  if (reqMidiStart) {                          // se reubicó el "1" con la intermitencia activa
    reqMidiStart = false;
    ticksSent = midiStartTick - 1;             // los ticks de antes del pulso no van
...
void renderBuffer() {
  ...
  if (reqResync) {
    reqResync = false;
    gateCount = 0; beatCount++;
    // El reloj MIDI cae en el mismo "1": este tick es el pulso. Con la intermitencia
    // activa se pide Stop+Start para que el sinte externo arranque acá mismo.
    tickAcc = 0;
    midiStartTick = midiTicks + 1;
    if (aGateOn) reqMidiStart = true;
    midiTicks++;
  }
  updateFilter();
  ...
  for (int i = 0; i < BUFFER_SAMPLES; i++) {
    ...
    // Reloj MIDI: 24 ticks por pulso, contados con el mismo contador que el corte
    // (acumulador entero: cero deriva entre el corte y el clock).
    tickAcc += MIDI_PPQN;
    if (tickAcc >= gatePeriod) { tickAcc -= gatePeriod; midiTicks++; }
